...
public class Expe {
	...
	public static void main(String[] args) throws Exception {
		...
		/*
		 * String ficName = "bench.txt"; int nbRes=3; BufferedReader readFile = new
		 * BufferedReader(new FileReader(ficName)); for(int nb=1 ; nb<=nbRes; nb++) {
		 * Model model=lireReseau(readFile); if(model==null) {
		 * System.out.println("Problème de lecture de fichier !\n"); return; }
		 * System.out.println("Réseau lu "+nb+" :\n"+model+"\n\n"); }
		 * 
		 * //Question6
		 * 
		 * //Premier jeu d'essai int nbReseauxSol1 = 0;
		 * 
		 * String ficName1 = "benchSatisf.txt"; int nbRes1=3; BufferedReader readFile1 =
		 * new BufferedReader(new FileReader(ficName1)); for(int nb=1 ; nb<=nbRes1;
		 * nb++) { Model model=lireReseau(readFile1); if(model==null) {
		 * System.out.println("Problème de lecture de fichier !\n"); return; } // Calcul
		 * de la première solution if(model.getSolver().solve()) { nbReseauxSol1++; }
		 * 
		 * } System.out.println("Le nombre de réseaux qui ont une solution " +
		 * nbReseauxSol1);
		 * 
		 * //Question 7
		 * 
		 * //Deuxieme jeu d'essai int nbReseauxSol2 = 0;
		 * 
		 * String ficName2 = "benchInsat.txt"; int nbRes2=3; BufferedReader readFile2 =
		 * new BufferedReader(new FileReader(ficName2)); for(int nb=1 ; nb<=nbRes2;
		 * nb++) { Model model=lireReseau(readFile2); if(model==null) {
		 * System.out.println("Problème de lecture de fichier !\n"); return; } // Calcul
		 * de la première solution if(model.getSolver().solve()) { nbReseauxSol2++; }
		 * 
		 * } System.out.println("Le nombre de réseaux qui ont une solution " +
		 * nbReseauxSol2); return;
		 */
